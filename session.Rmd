...
For this we are going to use the library [openxlsx](https://ycphs.github.io/openxlsx/)
...
```{r}
fac <- read_csv("data/facilities.csv")
rel <- read_csv("data/releases.csv")
```
...
As you dig into graphics with ggplot a whole new world opens up. [Cedric Scherer's tutorials](https://www.cedricscherer.com/tags/ggplot2/) are superb. Also, the website [R Graph Gallery](https://r-graph-gallery.com/) has a lot of examples of different chart types.
...
And if you are still on Twitter you may want to follow the hashtag #tidytuesday, where users share their graphs and codes.